---
title: "Project directories: gsource and the source verbs"
output:
    html_document:
        toc: true
        toc_depth: 3
...
vignette: >
    %\VignetteIndexEntry{Project directories: gsource and the source verbs}
...
A **gsource** is a managed project directory that gives GiottoDisk stores
a stable home on disk. It tracks artifacts in a JSON manifest, provides a
vault layout for storing files, and lets you snapshot a Giotto object to
durable storage with the artifacts it depends on intact.

This vignette walks through:

- the **project directory layout** a gsource manages
...
# Write a store; gsource allocates a uid and records the artifact
...
The directory is created on first use. Concurrent writers are safe — each
`sourceWrite` queues an atomic entry in `_pending/` rather than racing on
`giottodir.json` directly; the next read consolidates pending entries
into the manifest.
...
through the gsource see the artifact.
...
After `sourceWrite`, `pgs@uid` matches a manifest entry and
`pgs@path` points into the vault.
...
Brings a store under gsource management. The exact behaviour depends on
where the store currently lives and what type it is.

**`fileStore` (parquet stores)**: preserves the store's existing
`@uid`, moves files into `artifacts/<uid>/`, writes the manifest entry.

**`SpatRaster`**: allocates a fresh uid. On-disk rasters are
`file.copy`'d into the vault (`terra::rast()` re-opens at the new path);
in-memory rasters are written as COG.
...
that directory; adopting `normalized` would then fail with ENOENT
trying to read the old path.
...
transitive: if snapshot A references store X, and X depends on Y, both
X and Y are kept (BFS over the manifest's `depends` graph).
...
`giottosave/<name>.<ext>` plus the manifest tags that pin its
artifacts. Snapshots are how a giotto object survives across sessions
without breaking when the artifacts it references get relocated or
pruned.
...
their gsource backing — subsequent saves and prunes work the same way.
...
## Shared gsource across analyses

Multiple gobjects can share one gsource. This is the recommended
...
`sourcePrune` keeps the manifest tidy. A single gsource shared across
an entire lab would accumulate artifacts unboundedly.

## HPC / NFS: persistent dump

On HPC systems, the default `tempdir()`-based dump dies with the
session, which is wrong for long-running jobs. Point the dump at the
gsource's own vault:
...
|---------|------------------|
| Local / workstation | `tempdir()` (default) |
| NFS / HPC (SLURM)   | `setArtifactDumpDir(src)` |
| SLURM array jobs    | One gsource per sample (no shared-manifest contention) |
| Multi-analysis      | Single shared gsource |

## What survives a crash?

- `_pending/<uid>.json` writes are atomic (write to temp → rename). A
  crashed writer leaves nothing behind.
- `giottodir.json` consolidation is atomic (write to temp → rename
  over the old file). Either the old manifest or the new manifest is
  always intact, never a partial write.
...
*before* the snapshot file lands, the adopted artifacts are
"abandoned" — present in the vault and the manifest but not yet tagged
by any save. The next `sourcePrune` will reclaim them.
...
- `?gDirSource`, `?sourceWrite`, `?snapshotSave` etc. — generic-level
  reference docs.
