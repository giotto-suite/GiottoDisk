...
| `vignettes/chunking.Rmd` | **Installed vignette** (`vignette("chunking")`) — why bounded expression passes window the cell axis, when it happens, what it costs, and why not a spilling engine. The reference for windowing; `?storeChunkInfo` carries the options, adr/0011 the decision. |
